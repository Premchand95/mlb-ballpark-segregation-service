...
func UsageExamples() string {
	return os.Args[0] + ` scheduler index --body '{
      "date": "2022-08-10"
   }' --id 120` + "\n" +
		""
}
...
func ParseEndpoint(
	...
	var (
		...
		schedulerIndexBodyFlag = schedulerIndexFlags.String("body", "REQUIRED", "")
	...
	)
	{
		...
		case "scheduler":
			...
			case "index":
				endpoint = c.Index()
				data, err = schedulerc.BuildIndexPayload(*schedulerIndexBodyFlag, *schedulerIndexIDFlag)
			}
		}
	}
	if err != nil {
...
COMMAND:
    index: Retrieves a collection of licenses
...
func schedulerIndexUsage() {
	fmt.Fprintf(os.Stderr, `%[1]s [flags] scheduler index -body JSON -id UINT
